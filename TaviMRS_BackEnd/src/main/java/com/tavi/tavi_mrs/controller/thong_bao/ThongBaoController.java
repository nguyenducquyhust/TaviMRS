...
public class ThongBaoController {
    ...
    public ResponseEntity<JsonResult> findById(@RequestParam("thong-bao-id") int idThongBao) {
        return thongBaoService.findByIdThongBaoAndXoa(idThongBao, false)
    ...
    @GetMapping("/search")
    public ResponseEntity<JsonResult> findByMo(@RequestParam(name = "ngay-dau",  defaultValue = "1970-01-01T00:00:00+00:00", required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDate ngayDau,
        ...
        Page<ThongBao> thongBaoPage = thongBaoService.findByThoiGianGuiAndTieuDe( DateTimeUtils.asDate(ngayDau), DateTimeUtils.asDate(ngayCuoi),tieuDe,  pageable);
        return Optional.ofNullable(thongBaoPage)
    ...
    @PutMapping("/update")
    public ResponseEntity<JsonResult> update(@Valid @RequestBody ThongBao thongBao) {
